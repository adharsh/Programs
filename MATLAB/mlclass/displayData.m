function [h, display_array] = displayData(X, example_width)
	...
	%		size(display_array(pad + (j - 1) * (example_height + pad) + (1:example_height), pad + (i - 1) * (example_width + pad) + (1:example_width)))
	%		pause;
		% 20 x 20

	%	size(reshape(X(curr_ex, :), example_height, example_width) / max_val)
	%	pause;
		% 20 x 20
